var svg1_7 = d3.select("#svg-pos1-7"),
      margin = {top: 80, right: 90, bottom: 30, left: 65},
      width = +svg1_7.attr("width") - margin.left - margin.right,
      height = +svg1_7.attr("height") - margin.top - margin.bottom,
      g1_7 = svg1_7.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
    var color1_7 = d3.scaleOrdinal()
      .domain(["NV", "headache"])
      .range(["#00d02388", "#92a2c3","#ff794b"]);
    ...
    d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/main/NV_symptoms%2B%2By.csv", type, function(error, data) {
      ...
      z1_7.domain(sources.map(function(c) { return c.id; }));

      var source = g1_7.selectAll(".area")
        .data(array1)
        .enter().append("g")
          .attr("class", function(d) { return `area ${d.id}`;})
          .style("fill", function(d) { return z1_7(d.id); })
    
      source.append("path")
        .attr("d", function(d) { console.log(area1_7(d.values)); 
          return area1_7(d.values); })
        .style("fill", function(d) { return z1_7(d.id); });
      ...
      var mousemove = function(d) {
        tooltip
          ...
          .style("top", (d3.mouse(this)[1]) + margin.top + -40 + "px")
      }
      ...
      g1_7.append("text")
        ...
        .style("fill", "#111")
        .text("Side Symptoms of Nausea & Vomiting");
    });
...
var svg1_7 = d3.select("#svg-pos1-7"),
    ...
    var color1_7 = d3.scaleOrdinal()
      ...
      .range(["#00d02388", "#92a2c300", "#ff794bcc",  "#0000", "#0000", "#0000"]);
       //https://www.w3schools.com/colors/colors_names.asp
    ...
    d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/main/NV_symptoms%2B%2By.csv", type, function(error, data) {
      ...
      source.append("path")
          .attr("d", function(d) { console.log(area1_7(d.values)); return area1_7(d.values); })
          .style("fill", function(d) { return z1_7(d.id); });
