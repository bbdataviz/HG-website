var svg1_9 = d3.select("#svg-pos1-9"),
      margin = {top: 80, right: 90, bottom: 30, left: 65},
      width = +svg1_9.attr("width") - margin.left - margin.right,
      height = +svg1_9.attr("height") - margin.top - margin.bottom,
      g1_9 = svg1_9.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    var color1_9 = d3.scaleOrdinal()
      .domain(["NV", "fatigue", "headache", "sleep", "depress", "hospital"])
      .range(["#00d02388", "#92a2c3", "#ff794bed",  "url(#gradient01)", "#1119"]);
    ...
    z1_9.domain(sources.map(function(c) { return c.id; }));

    var source = g1_9.selectAll(".area")
      .data(array1)
      .enter().append("g")
        .attr("class", function(d) { return `area ${d.id}`; })
        .style("fill", function(d) { return z1_9(d.id); })

    source.append("path")
      .attr("d", function(d) { console.log(area1_9(d.values)); 
        return area1_9(d.values); })
      .style("fill", function(d) { return z1_9(d.id); });
