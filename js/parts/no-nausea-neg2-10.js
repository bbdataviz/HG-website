var svg2_10_4 = d3.select("#svg-neg2-10"),
  margin = {top: 80, right: 90, bottom: 30, left: 65},
  width = +svg2_10_4.attr("width") - margin.left - margin.right,
  height = +svg2_10_4.attr("height") - margin.top - margin.bottom,
  g2_10_4 = svg2_10_4.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
var color2_10_4 = d3.scaleOrdinal()
...
d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/refs/heads/main/2-Neither%20Nausea%20nor%20Vomiting", type, function(error, data) {
  ...
  z2_10_4.domain(sources.map(function(c) { return c.id; }));

  var source = g2_10_4.selectAll(".area")
    .data(array1)
    .enter().append("g")
      .attr("class", function(d) { return `area ${d.id}`; })
      .style("fill", function(d) { return z2_10_4(d.id); })

  source.append("path")
    .attr("d", function(d) { console.log(area2_10_4(d.values)); 
      return area2_10_4(d.values); })
   .style("fill", function(d) { 
      if(d == sources["3"]) 
        return "url(#gradient01)"  //"#0022ff85" blue
      else return z2_10_4(d.id); });
  ...
  var mousemove = function(d) {
    tooltip
      ...
      .style("top", (d3.mouse(this)[1]) + margin.top + 330 + "px") 
  }
