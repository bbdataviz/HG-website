var svg12_10_2 = d3.select("#svg-neg1-10"),
  margin = {top: 80, right: 90, bottom: 30, left: 65},
  width = +svg12_10_2.attr("width") - margin.left - margin.right,
  height = +svg12_10_2.attr("height") - margin.top - margin.bottom,
  g12_10_2 = svg12_10_2.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
var color12_10_2 = d3.scaleOrdinal()
  .domain(["fatigue", "headache", "sleep", "depress"])
  .range(["#92a2c3","#ff794b", "#0022ff50", "#1119"]);  // "url(#gradient01)"
    //https://www.w3schools.com/colors/colors_names.asp
  var x = d3.scaleLinear().range([0, width]),
  y = d3.scaleLinear().range([height, 0]),
  z12_10_2 = color12_10_2;

var area12_10_2 = d3.area()
...
d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/main/hg-neg1", type, function(error, data) {
  ...
  z12_10_2.domain(sources.map(function(c) { return c.id; }));

  var source = g12_10_2.selectAll(".area")
    .data(array1)
    .enter().append("g")
      .attr("class", function(d) { return `area ${d.id}`; })
      .style("fill", function(d) { return z12_10_2(d.id); })

  source.append("path")
    .attr("d", function(d) { console.log(area12_10_2(d.values)); 
      return area12_10_2(d.values); })
    .style("fill", function(d) { 
      if(d == sources["2"]) 
        return "url(#gradient01)"  //"#0022ff85" blue
      else return z12_10_2(d.id); });
