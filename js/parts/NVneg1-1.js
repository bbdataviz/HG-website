var svg12_1 = d3.select("#svg-neg1-1"),
      margin = {top: 80, right: 90, bottom: 30, left: 65},
      width = +svg12_1.attr("width") - margin.left - margin.right,
      height = +svg12_1.attr("height") - margin.top - margin.bottom,
      g12_1 = svg12_1.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
    var color12_1 = d3.scaleOrdinal()
      .domain(["NV", "fatigue", "headache", "sleep", "depress"])
      .range(["#92a2c3","#92a2c3","#0000","#0000"]); //"url(#gradient_ocean)" transparnt: "#92a2c390","#92a2c3aa"
       //https://www.w3schools.com/colors/colors_names.asp
      var x = d3.scaleLinear().range([0, width]),
      y = d3.scaleLinear().range([height, 0]),
      z12_1 = color12_1;
    
    var area12_1 = d3.area()
    ...
    d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/main/hg-neg1", type, function(error, data) {
      ...
      z12_1.domain(sources.map(function(c) { return c.id; }));
    
      var source = g12_1.selectAll(".area")
        .data(array1)
        .enter().append("g")
          .attr("class", function(d) { return `area ${d.id}`; })
          .style("fill", function(d) { return z12_1(d.id); })
    
      source.append("path")
           .attr("d", function(d) { console.log(area12_1(d.values)); 
            return area12_1(d.values); })
           .style("fill", function(d) { return z12_1(d.id); });
    ...
    // Three function that change the tooltip+interaction feedback when user hover / move / leave a cell
      ...
      g12_1.append("g")
          ...
          .call(d3.axisLeft(y).tickValues([20, 40, 60, 80, 100]))
