var svg1_8 = d3.select("#svg-pos1-8"),
      margin = {top: 80, right: 90, bottom: 30, left: 65},
      width = +svg1_8.attr("width") - margin.left - margin.right,
      height = +svg1_8.attr("height") - margin.top - margin.bottom,
      g1_8 = svg1_8.append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    
    var color1_8 = d3.scaleOrdinal()
      .domain(["NV", "fatigue", "headache", "sleep", "depress", "hospital"])
      .range(["#00d02388", "#92a2c3", "#ff794bed",  "url(#gradient01)", "#1119", "#0000"]);
    ...
    d3.csv("https://raw.githubusercontent.com/bbdataviz/HG-D3/main/NV_symptoms%2B%2By.csv", type, function(error, data) {
      ...
      z1_8.domain(sources.map(function(c) { return c.id; }));

      var source = g1_8.selectAll(".area")
        .data(array1)
        .enter().append("g")
          .attr("class", function(d) { return `area ${d.id}`; })
          .style("fill", function(d) { return z1_8(d.id); })
    
      source.append("path")
        .attr("d", function(d) { console.log(area1_8(d.values)); 
          return area1_8(d.values); })
        .style("fill", function(d) { return z1_8(d.id); });
